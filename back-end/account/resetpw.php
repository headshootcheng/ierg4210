<?php
    ...
    if(empty($inputoldpassword)||empty($inputnewpassword)||empty($inputnewpassword2)){
        header("Location: ../../resetpw.php?error=* Please fill in all the fields");
        exit;
    }
    if(empty($_REQUEST['csrftoken'])){
        header("Location: ../../resetpw.php?error=* Unexpected Error");
        exit;
    }
    if($inputnewpassword!=$inputnewpassword2){
        header("Location: ../../resetpw.php?error=* 2 fields must be the same");
        exit;
    }
    else{
        ...
        foreach($result as $row){
            if(!password_verify($inputoldpassword,$row["password"])){
                header("Location: ../../resetpw.php?error=* Wrong passoword");
                exit;
            }
        }
        $hash=password_hash($inputnewpassword,PASSWORD_DEFAULT,['cost' => 11]);
        ...
        unset($_SESSION['user']);
        session_destroy();
        header("Location: ../../login.php?success=* Your Password is successfully updated ");
        exit;
    }
?>